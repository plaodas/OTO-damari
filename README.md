...
| 強い | 174 / 285 / 963 Hz の和音 | 高速で流れて光の帯になる |
| タップ | ソルフェジオ共鳴音 | 周囲の粒子が弾ける |
| 左下の丸 | カメラ | 撮った像の暗い部分ほど粒子が密に集まり、被写体が点描として浮かぶ |
| 傾ける | — | 低い方向へゆっくり漂う |
| 北を向く | — | 粒が上へ流れ始める（地磁気） |

- 吹く強さが一段上がった瞬間に共鳴音が鳴ります。強い吹きのあいだは 174 / 285 / 963 Hz の和音が流れ続けます
- マイクを拒否すると吹きかけ検出だけが無効になり、粒子表示とタップ操作は継続します
- タップ、スワイプ、シェイク、吹き、撮影、北向きへの進入では、174〜963 Hzの9音が控えめな共鳴音として順番に巡ります。前の音は残したまま最大5音まで重なります。音響演出であり、医療・治癒効果を意図したものではありません
...
粒子の位置と速度は GPU バッファで ping-pong します。描画は WebGL2 の point sprite（または instanced quad）と生成ノイズテクスチャを使用し、`gl.ONE, gl.ONE` の加算合成で発光させています。強吹き時だけ 1 フレーム遅れの位置で短い光跡を `gl.LINES` で重ねます。マイク RMS とスワイプは速度場への splat、漏斗は場への外力です。吹き・タップ・スワイプの音はソルフェジオ共鳴音です。
